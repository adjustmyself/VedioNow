...
async function migrateMongoToSqlite(mongoConnectionString, sqliteDbPath) {
    ...
    try {
        ...
        const insertTag = db.prepare(`
            INSERT INTO tags (name, color, description, description_image, group_id, sort_order, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(name) DO UPDATE SET color = excluded.color, description = excluded.description, description_image = excluded.description_image, group_id = excluded.group_id, sort_order = excluded.sort_order
        `);
        for (const t of tags) {
            const groupId = t.group_id ? (groupIdMap.get(t.group_id.toString()) || null) : null;
            insertTag.run(t.name, t.color || '#3b82f6', t.description || '', t.description_image || '', groupId, t.sort_order || 0, toIso(t.created_at), toIso(t.updated_at));
            counts.tags++;
        }
